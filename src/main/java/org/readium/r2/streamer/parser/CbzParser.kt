...
class CbzParser : PublicationParser {
    ...
    override fun parse(fileAtPath: String, title: String): PubBox? {
        val container = try {
            ...
            println("Error: Could not generate container $e")
            return null
        }
        val listFiles = try {
            container.getFilesList()
        } catch (e: Exception) {
            println("Error: Missing File : META-INF/container.xml $e")
            return null
        }
...
//        publication.metadata.title = container.getTitle()
    ...
    private fun getMimeType(fileName: String): String? {
        return try {
            ...
            Files.probeContentType(File(name).toPath())
        } catch (e: Exception) {
            println("Error: Something went wrong while getMimeType() : ${e.message}")
            null
        }
    }
...
//        } catch (e: Exception){
//            println("Error", "Can't open the file (${e.message})")
//        }
//        return listCBZ
//    }
